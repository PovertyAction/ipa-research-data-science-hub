...
> **NOTE:**
>
> Our lessons store their data files in a `data` sub-directory, which is why the path to the file is `data/gapminder_gdp_oceania.csv`. If you forget to include `data/`, or if you include it but your copy of the file is somewhere else, you will get a [runtime error](../../data-science/data-analysis/built-in.llms.md) that ends with a line like this:
>
> ``` error
...
> > **TIP:**
> >
> > 1.  We can check out the first five rows of `data_americas` by executing `data_americas.head()` which allows us to view the beginning of the DataFrame. We can specify the number of rows we wish to see by specifying the parameter `n` in our call to `data_americas.head()`. To view the first three rows, execute:
> >
> > ``` python
...
> ```
>
> What values should you pass to `read_csv` to read `microbes.csv` in `analysis.ipynb`?
>
> > **TIP:**
> >
