...
abstract: "Understand the grammar of graphics framework. Learn how to map data to visual properties. Explore different aesthetic mappings beyond x and y coordinates."
authors-ipa:
...
The **grammar of graphics** is a systematic approach to creating data
visualizations. Instead of thinking about "chart types" (bar chart, scatter
plot, etc.), we think about the **components** that make up any visualization.
...
When we create a visualization, we're **encoding** data as visual properties.
The main aesthetic properties we can use are:
...
Color is one of the most powerful ways to encode information. Let's see how it
works:
...
Notice how the color gradient shows the range of body mass values. Lighter or
darker colors indicate higher or lower values.
...
- **Diverging data** (change from baseline, profit/loss) → Use diverging
  palettes (e.g., blue-white-red)
:::
...
Larger dots represent penguins with greater body mass. This creates a "bubble
plot" where we're visualizing three variables simultaneously!
...
Shape (called `marker` in seaborn.objects) is useful for categorical variables,
especially when printing in black and white:
...
Don't use shape for variables with many categories (more than 6-7). It becomes
hard to distinguish between shapes. For many categories, use color instead.
:::

## Combining Color and Shape

For maximum clarity and accessibility (including for colorblind viewers),
...
Now species are distinguished by both color AND shape - making the plot
...
Where many points overlap, the color becomes darker, showing areas of high
density.

## Research Example: Education and Income Data

Let's imagine we have data from a household survey in Kenya. Here's how we might
...
What patterns do you see? Do males and females differ in body mass? Do penguins
from different islands cluster differently?
:::::
:::
...
Then discuss: Is this plot easy to understand? When does adding more aesthetics
...
**Discussion**: While technically you can encode many variables, there's a limit
to how much information people can process visually. Generally:

- 3-4 variables is very manageable
- 5 variables can work if the relationships are clear
- 6+ variables usually creates confusion

Consider whether faceting (small multiples) might be clearer than packing
everything into one plot.
:::::
:::
...
Think about a dataset from your own research work. Sketch out (on paper or
describe in words) a visualization that would show:
...
Discuss with a partner: Would stakeholders understand this visualization? What
...
1. **Rainbow color schemes** for continuous data - they're not perceptually
   uniform and are problematic for colorblind viewers

2. **Too many aesthetics** - More isn't always better. Keep it simple enough to
   understand.

3. **Inappropriate mappings** - Don't use size for categorical data or shape for
   continuous data

4. **No legend** - Always ensure viewers can decode your aesthetic mappings

5. **Inconsistent mappings** - If you use color for species in one plot, use the
   same mapping in related plots
:::
...
- Main aesthetics: position (x, y), color, size (pointsize), shape (marker), and
  alpha
...
In the next lesson, we'll explore different types of marks (geometric objects)
beyond dots - including lines, bars, areas, and bands. We'll learn how to choose
the right mark for your data and research questions.
:::
