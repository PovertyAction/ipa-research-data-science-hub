...
* **Define the timeframe, format, and structure needed**: "Primary school records from January 2022 to December 2023 in CSV format"
* **List specific variables of interest**: Student ID, school level, Teacher ID, attendance, test scores
* **Specify whether identified or de-identified data is required**: Request de-identified data when possible to reduce ethical complexity
* **Avoid broad requests**: Instead of "all student data," specify exact variables, timeframes needed, and frequency of updates
:::
...
1. **Gather Identifying Information**: Determine what identifiers are available in the study sample (e.g., national ID numbers, phone numbers, email addresses).
2. **Link Data Sets**: Use pre-existing identifiers to match study data with administrative data. For example, match student IDs with test records using national ID numbers.
3. **Choose a Matching Strategy**:
   * **Exact matching**: When identifiers are identical (e.g., national ID numbers)
