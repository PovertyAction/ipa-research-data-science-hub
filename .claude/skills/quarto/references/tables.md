...
Quarto supports multiple table formats including pipe tables, list tables, and
computational tables with extensive styling options.
...
Use bullet lists where top-level items (`-`) are columns and nested items are
rows:
...
List tables also support row/cell attributes (`[]{.highlight}`, `[]{align=r}`),
empty cells (lone `-`), and any markdown content in cells.
...
- **knitr engine**: Recognised table objects are rendered automatically (e.g. a
  data frame is printed as a markdown table without extra configuration).
- **jupyter engine**: Cells that return a display-protocol object auto-display
  as HTML in HTML output only. For portable output across all formats, print a
  markdown table string and set `output: asis`:
...
Combine multiple classes: `::: {#tbl-name .striped .hover .bordered}`. Use
`classes: plain` in code cells to disable default striping.

Quarto also processes HTML tables with `data-qmd` attribute for markdown
content. Disable with `html-table-processing: none`.
...
For tables spanning multiple pages (PDF), use a longtable option appropriate to
your engine/package.
...
- [Table
  Cross-References](https://quarto.org/docs/authoring/cross-references.html#tables)
