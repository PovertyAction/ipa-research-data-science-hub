...
Most R Markdown documents can be rendered by Quarto with minimal changes. The
...
Quarto doesn't require `rmarkdown` or `knitr`, but `knitr` remains useful for
tables and chunk processing. Most R Markdown features (`knitr::kable()`,
`knitr::include_graphics()`) work in Quarto without changes.

Note: Quarto can render `.Rmd` files directly (`quarto render document.Rmd`)
using R Markdown compatibility mode, which allows incremental migration.
...
- [Quarto vs R
  Markdown](https://quarto.org/docs/faq/rmarkdown.html#quarto-vs.-r-markdown)
