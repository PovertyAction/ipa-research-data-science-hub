...
Quarto natively supports Mermaid and Graphviz diagrams, rendering them
automatically across output formats.
...
All standard Mermaid diagram types are supported: `sequenceDiagram`,
`classDiagram`, `stateDiagram-v2`, `erDiagram`, `gantt`, `pie`, etc. Use
standard Mermaid syntax inside `{mermaid}` code cells.
...
Use `digraph` for directed graphs, `graph` for undirected. Standard DOT features
(subgraphs, node styling, rank direction) all work.
...
Use `%%| fig-width` and `%%| fig-height` cell options. Diagrams are responsive
by default in HTML; disable with `%%| fig-responsive: false`.
...
When using `mermaid-format: js` (the default for HTML), Quarto controls theming
and may override custom theme configurations. The YAML config block inside the
Mermaid cell might appear to have no effect.
...
With `svg` or `png` format, the YAML config block inside Mermaid cells will be
respected.
