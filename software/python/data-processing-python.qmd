...
The `pandas` library (abbreviated as `pd` by convention) is the primary tool for data manipulation in Python. The `causaldata` package provides easy access to several datasets commonly used in statistics and research, including Gapminder. If you do not have these libraries installed in your [python virtual environment](../virtualenv/index.qmd), you can add them by running `uv add pandas causaldata` from your terminal.
