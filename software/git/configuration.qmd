...
use different characters to represent the end of a line.
...
For versions of Git prior to 2.28, the change can be made on an individual repository level.
Note that if this value is unset in your local Git configuration, the `init.defaultBranch`
value defaults to `master`.

Since we are creating the repository on GitHub, the default branch name for the
`multiverse` repo will be `main`. Configure this setting if you plan on creating git repositories locally first.
:::
