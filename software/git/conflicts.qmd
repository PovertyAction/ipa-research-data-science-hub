...
But before we checkout the `loki-twist` branch and add the event where Loki enters
the picture, let's add a line to `earth.txt` here in the `main` branch.
...
![Merge conflict in earth.txt](/assets/images/git/vscode-merge-conflict.png)
...
![The bottom pane contains the resolved earth.txt contents](/assets/images/git/vscode-merge-resolved.png)

By clicking "Complete Merge", you are doing the same thing as running the command `git add earth.txt`.
If you open the Source Control pane, you should see `earth.txt` under "Staged Changes". You can then
...
conflicting change (e.g. adding two different lines to the end of `earth.txt`).
