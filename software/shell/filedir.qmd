...
Here,
the computer's response shows Amara's **home directory** (the exact path format differs by operating system and shell).
...
These are a sample of the files Amara needs to validate. Each `hh_baseline_*.csv` file contains household survey data,
and the `validate_survey.*` scripts (which we'll learn to build later) can check them for common issues. In reality, Amara has 1520 such files to validate, we're only showing a few here for demonstration.
