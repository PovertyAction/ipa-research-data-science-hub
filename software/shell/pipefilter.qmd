...
Let's go into that directory with `cd` and run an example command `wc` (word count) to count lines in a survey file:
...
These piped commands show how we can combine simple tools to perform data validation—exactly what Amara needs for her survey quality checks.
