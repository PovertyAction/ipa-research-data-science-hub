...
``` stata
  ...
  */
  generate flag_before1 = _n if when == 1
  ...
  */
  generate flag_before2 = _n if when == 1
  /*
...
1.  Use the option `stable` to make sure Stata uses the same randomization every time

2.  You cannot use this option with \`gsort’

3.  The preferred method is to specify a combination of variables that uniquely identifies your observations. This removes the randomization and makes your sort outcome be exactly what you specify and expect
...
## `bysort`, `generate`, and `egen`

`bysort` combined with generate/egen is probably one of the most useful command combinations when cleaning and creating outcomes.
...
3.  Pay attention to whether the function you are using needs to specify `generate` or `egen`

&nbsp;

1.  Notice that `sum` works for both generate and egen (even though it is not in the `egen` documentation and works differently

- egen + sum = creates a total for all values specified in the by
- generate + sum = creates a cumulative sum over the observations specified
