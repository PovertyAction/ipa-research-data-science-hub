...
Learn about WhatsApp templates and how to create them effectively.
:::
...
## Key Takeaways
* Delve into Twilio's menus and options to navigate its functionalities effortlessly, utilizing and creating basic widgets within a flow and leveraging functions for seamless integration. 
* By the end, you'll be well-equipped to publish and effectively manage information collected through WhatsApp Surveys.
:::
...
![Create a new content template](/assets/images/twilio-design-17.png){fig-cap="Create a new content template" width=100%}
...
1. **text (Text):** A standard text message containing only plain text.
2. **media (Media):** A message that includes images, videos, or audio files.
3. **quick-reply (Quick Reply):** A message with predefined response buttons for users to select from.
4. **call-to-action (Call to Action):** A message with buttons that direct users to a link or initiate a phone call.
5. **list-picker (List Picker):** A structured message that presents users with a selectable list of options.
6. **card (Card):** A message format that includes an image, title, description, and optional buttons for interaction.
7. **card (WhatsApp Card):** A WhatsApp-specific card message with rich media and interactive elements.
8. **authentication (Authentication):** A message type designed for sending authentication codes.
9. **catalog (Catalog):** A message format for showcasing multiple items in a structured catalog layout.

![New content template](/assets/images/twilio-design-18.gif){fig-cap="Create a new content template" width=110%}
...
![Create a new Twilio flow](/assets/images/twilio-design-5.gif){fig-cap="Create a new Twilio flow" width=100%}
...
![Send & Wait Widget Example](/assets/images/twilio-design-9.png){fig-cap="Send & Wait Widget Example" width=80%}
...
![Create a New Service](/assets/images/twilio-design-15.png){fig-cap="Create a New Service" width=100%}
...
This function will be covered in detail in the [Whatsapp survey deploy](whatsapp-deploy.qmd) section.
...
This function will be covered in detail in the [Whatsapp data security](whatsapp-data-security.qmd) section.
