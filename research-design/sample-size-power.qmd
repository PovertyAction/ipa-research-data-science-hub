...
PDFs help visualize the uncertainty around the estimated treatment effect ($\hat{\beta}$) in a study. They show how likely different values of $\hat{\beta}$ are, assuming a true treatment effect $\beta$.
...
A narrower distribution of estimates ($\hat{\beta}$) increases the chances of detecting true effects. You can achieve this by:
...
If researchers repeated the same RCT many times, they wouldn't get the exact same estimate every time. Instead, the estimated treatment effects ($\beta$) would follow a normal distribution:
...
- A lower $\alpha$ reduces false positives—but increases the risk of missing real effects
- Its complement (1−$\alpha$) is known as the confidence level -- how likely studies are to avoid a false positive
...
Proposes existence of an effect, $\beta \neq 0$. Can be one-sided, $\beta > 0$ or $\beta < 0$, or two-sided, $\beta \neq 0$.
...
Based on chosen significance level $\alpha$, typically uses $\alpha = 0.05$. Forms rejection regions in the distribution and uses t-distribution or z-distribution.
...
Calculate estimated effect size $\hat{\beta}$ and compare to critical values. Consider confidence intervals and assess statistical significance.
:::
...
Reject $H_0$ if $\hat{\beta}$ exceeds critical values. Fail to reject $H_0$ if $\hat{\beta}$ falls within critical bounds. Consider practical significance and report p-values and confidence intervals.
