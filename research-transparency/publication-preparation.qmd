...
| Level | Materials Included | Enables | Recommended Formats |
| ----- | ------------------ | ------- | ------------------- |
| **Minimum** | Data and code underlying published results, ReadMe explaining file relationships, minimal study-level metadata | Verifying that tables in the published article can be reproduced by running the code; limited reuse | Data: `.dta`, `.csv` + codebook; Code: `.do`, `.py`, `.r`; ReadMe: `.pdf`, `.md`; Surveys: `.pdf`, `.xlsx` |
| **IPA-Recommended** | Minimum level, plus full set of collected variables (excluding PII), detailed data documentation, survey instruments | Fuller potential for reuse and secondary analysis; better understanding of study context for systematic reviews and policy application | Same as above |
| **Exceptional** | IPA-Recommended level, plus cleaning and variable construction code | Start-to-finish reproducibility | Same as above |
...
IPA maintains a data repository using Harvard's Dataverse at [dataverse.harvard.edu/dataverse/IPA](https://dataverse.harvard.edu/dataverse/IPA). For step-by-step instructions on curating and uploading a replication package, see [How to Publish a Replication Package to Dataverse](how-to-publish-dataverse.qmd).
