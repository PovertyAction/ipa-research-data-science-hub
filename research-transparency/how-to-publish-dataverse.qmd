---
title: "How to Publish a Replication Package to Dataverse"

abstract: "This how-to guide provides step-by-step instructions for curating research materials and publishing a replication package to IPA's Dataverse."
...
- Set up a curation workspace with separate folders for original files, working
  files, and final Dataverse files.
- Remove all personally identifiable information using IPA's PII detection tools
  and document your decisions.
- Verify that all code runs without errors and produces outputs matching
  published results.
...
This guide walks you through the data curation workflow for publishing a
replication package to [IPA's
Dataverse](https://dataverse.harvard.edu/dataverse/ipa). Before starting, ensure
you have:
...
For background on why data sharing matters, see [Understanding Research
Transparency](understanding-transparency.qmd). For detailed requirements on what
materials to include, see [Data Publication
Preparation](publication-preparation.qmd).
:::

## Step 1: Set Up Your Curation Workspace

Create a folder structure that preserves original files while allowing you to
work on modifications.

### Create the Folder Structure

Within your replication folder (named with the study title and primary PI),
...
![Folder Structure](../assets/images/dataverse/dataverse_folders.png)

The "Working Files" and "Dataverse Files" folders should use the following
structure:
...
All datasets must be checked for and stripped of personally identifiable
information before publication.

::: {.callout-important}
## Comprehensive PII Guidance

For detailed information on identifying PII, including HIPAA guidelines,
detection strategies, and anonymization techniques, see [Data Security Protocol:
Personally Identifiable
Information](../data-security/data-security-protocol.html#personally-identifiable-information-pii).
:::
...
1. **PII Detection Tool**: A web-based tool that checks datasets one at a time.
   Upload each dataset and review the flagged variables. See [PII Detection
   Tool](https://github.com/PovertyAction/PII_detection) on GitHub for
   instructions.

2. **PII Flagging Code**: A Stata do-file that outputs flagged variables to a
   log file. You can process multiple datasets by listing them in the local
   macro. See [split_pii](https://github.com/PovertyAction/split_pii) on GitHub
   for instructions.
...
- **Encode** variables used in analysis that contain identifiable information
  (anonymize variable and value labels too)
- **Modify specific entries** in free-text fields (`_spec` and `_oth` variables)
  rather than dropping the entire variable when only some responses contain PII

::: {.callout-tip}
## Handling Free-Text Responses

Variables with `_spec` and `_oth` suffixes often contain free-form responses.
Review these with care---they may contain PII within participant responses such
as names, locations, and contact details. Modify only the entries containing PII
rather than dropping the entire variable, as these responses are often
informative.
:::
...
After anonymizing datasets, verify that all analysis code runs through without
breaking.
...
Run each do-file sequentially, fixing breakage as you encounter it. Common
sources of code breakage include:
...
::: {.callout-warning}
Be cautious with modifications. Ensure changes do not alter result outputs in
ways that create discrepancies between code output and reported results.
:::
...
IPA defines a discrepancy as any difference greater than 0.001 between code
output and published results.
...
See [Data Publication
Preparation](publication-preparation.qmd#readme-file-structure) for a complete
ReadMe template.
...
The peer reviewer should also conduct a secondary PII check to ensure no
identifiable information remains in the datasets.
...
4. Remove the "(2)" from the final filename; filenames should be brief,
   descriptive, without unnecessary suffixes, and avoid spaces (use underscores,
   hyphens)

::: {.callout-note}
## Why Double Compression?

Double compression ensures the folder structure remains intact when users
download and unzip the files from Dataverse.
:::
...
![Add New Dataset](../assets/images/dataverse/dataverse_new_dataset.png)

### Enter Initial Metadata

Fill in the metadata fields using information from the project page and
publication:
...
![Save Dataset](../assets/images/dataverse/dataverse_save_dataset.png)
...
![Edit Terms](../assets/images/dataverse/dataverse_edit_terms.png)
...
![Publish Dataset](../assets/images/dataverse/dataverse_publish_dataset.png)
...
- [Understanding Research Transparency](understanding-transparency.qmd)
- [Data Publication Preparation](publication-preparation.qmd)
- [Data Security
  Protocol](https://data.poverty-action.org/data-security/data-security-protocol.html)
