...
This page provides IPA staff and partners with a practical overview of standards
for protecting sensitive research data. It covers PII handling, password and
device management, encryption (SurveyCTO and Cryptomator), AI usage, IRB
compliance, and secure backup practices. A template for creating project-level
Data Security Plans is included.
:::

::: {.callout-note collapse="true"}
## Implementation Guide

**Ready to implement these protocols?** This page provides the standards and
principles. For step-by-step implementation instructions, role-specific
checklists, and emergency procedures, see the [How to Implement Data Security
Protocols](../data-security/how-to-data-security.qmd) guide.
:::

::: {.callout-tip appearance="simple"}
## Key Takeaways

- Protecting sensitive data and PII is essential to maintain respondent
  confidentiality and comply with ethical standards.
- Implementing robust encryption, password security, and backup practices
  ensures data integrity and prevents unauthorized access.
- Device security and proper data flow management are critical components of
...
- **Respect for respondents:** Human subjects research data reflects real issues
  affecting real people.
- **Informed consent:** Respondents trust researchers to protect their
  confidentiality. Breaches carry consequences.
- **Compliance:** Required by IRBs, donors, and laws (e.g., Common Rule, HIPAA,
  GDPR).
- **Increasing risks:** Hacks, phishing, device theft, and accidental sharing
  can compromise data.
- **Human error:** Weak passwords, reused logins, or overlooked identifiers can
  re-identify individuals:contentReference[oaicite:1]{index=1}.

--------------------------------------------------------------------------------

## Personally Identifiable Information (PII)

PII refers to any datapoint or combination of datapoints that can identify an
individual or household with reasonable certainty.
...
> employment information." ([NIST SP
> 800-122](https://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-122.pdf))
...
- Social security numbers, medical record numbers, health plan beneficiary
  numbers
- Account numbers, certificate or license numbers, vehicle identifiers, device
  identifiers
- Web URLs, IP addresses, biometric identifiers like fingerprints
- Full-face photos or any other unique identifying number, characteristic, or
  code
:::
...
- **Encryption:** Apply encryption at every stage of the data lifecycle:
...
- Sharing PII without proper encryption or
  deletion:contentReference[oaicite:2]{index=2}

### PII vs. Sensitive Data

There is an important distinction between PII and sensitive data:

- **Sensitive data** (e.g., sexual orientation, health status, income) is not
  inherently identifying but becomes problematic when linked to PII
- **PII** can directly identify individuals or when combined with other data
- Sensitive data without PII linkage poses no confidentiality threat to
  respondents
...
- Approved individuals must complete **human subjects protection training** like
...
The certification ensures research staff understand ethical research principles
and data security importance.

## Sensitive Data Classification

Sensitive data is information where a loss of confidentiality, integrity, or
availability could result in serious, severe, or catastrophic consequences.
...
- Avoid repetition, dictionary words, usernames, pronouns, IDs, and predefined
  sequences.
- Use **strong passwords**: ≥10 characters, mixed types, avoid common
  substitutions.
...
- Encrypts data **at point of capture**, ensuring protection both in transit and
  at rest.
...
- [SurveyCTO encryption
  guidance](https://docs.surveycto.com/02-designing-forms/02-additional-topics/06.encrypting.html)

### Cryptomator

- IPA's chosen open source software for encrypting files locally and on cloud
  services (Box, Dropbox).
- Uses 256-bit AES encryption with Advanced Encryption Standard, encrypting both
  files and filenames.
- **Authentication**: IPA staff access through Microsoft Entra SSO; external
  partners receive username/password.
- **Vault Management**: Automated vault creation and permissions managed through
  Salesforce project roles.

**Critical Security Requirements:**

- Store your **Account Key** securely in approved password manager (required for
...
- For external partners: Store Account Key in password vault (e.g., Keeper,
  LastPass, 1Password)

### Device / Cloud / Laptop Standards

- **Devices:** Full-disk encryption; enable remote wipe (e.g., Android, Google
  accounts).
- **Cloud:** Encrypt synced data using Cryptomator.
- **Laptops:** Use full-disk encryption, separate admin/user profiles, disable
  unused USBs, and install antivirus.
- **Box Integration:** Install Box Drive for seamless Cryptomator vault access.
- **Multi-device Access:** Authorize new devices using Account Key through
  Cryptomator Hub profile page.
...
- **Separate immediately**: PII sheets can be physically separated from survey
  content
...
- **Create electronic backups**: Scan and encrypt paper forms when possible
  (paper is vulnerable to fire, flood, theft)
...
- **Never upload PII** or staff data into online AI services unless explicitly
  approved by the IRB and institutional guidelines
- **Never input confidential information or intellectual property** such as
  grant proposals or non-public working papers unless explicitly approved by the
  content owner
- **Never share system credentials** with AI engines

### Before Using AI Tools

- **Always review data use, contractual, and grant agreements** of any AI tool
  before using it
- **Always review privacy agreements** of any AI tool before using it
- **Verify data handling policies** to ensure compliance with organizational
  standards
- **Confirm tool approval** with your supervisor for new AI services

*Keep in mind: AI is a tool for generating and drafting content. Always edit and
...
Every project should prepare a Data Security Plan (DSP). Below is a suggested
format:
...
By adhering to these data security protocols, researchers ensure compliance,
safeguard respondent privacy, and maintain the integrity of research data.
...
- [ ] Never share PII or confidential content with AI tools, unless approved by
      IRB and institutional guidelines
